...
function tooBigToWorry(food, eater) {
	return food.mass * 6 < eater.mass && (!eater.name || enemyList.length < 1 || enemyList.indexOf(eater.name) == -1);
}
...
function createEdgeThreat(myOrganism) {
	...
	var threatArray = [];
	if (myOrganism.ox < threatDistance - 7055) {
	...
	if (myOrganism.oy < threatDistance - 7055) {
